...
[CreateAssetMenu(fileName = "swCharge", menuName = "CommandData/swCharge")]
public class swCharge : CommandData
{
    ...
    public override void ActivateSkill(GameObject castPoint, GameObject target)
    {
        ...
        if (effectPrefab != null)
        {
            for (int i = 0; i < initCount; i++) // 원거리 이펙트 개수만큼 생성 및 발사
            {
                ...
                projectileScript.Initialize(target, damage, projectileSpeed, destroyTime, true); // 투사체 스크립트에 필요한 정보(타겟, 데미지, 투사체속도, 유지시간, 유도 T/F) 부여, 유도할 것이면 T, 그냥 투사체면 F
